...
//	}
	...
	elm_naviframe_item_push(nf, "Layout", NULL, NULL, main_layout, NULL);
	elm_box_align_set(main_layout, 0.5, 0.5);

	Evas_Object *laundry_box_wrapper = elm_layout_add(main_layout);
	elm_layout_file_set(laundry_box_wrapper, edj_path, "box_wrapper");
	elm_layout_content_set(main_layout, "elm.swallow.content", laundry_box_wrapper);
	evas_object_size_hint_max_set(laundry_box_wrapper, 0, 0);
	...
	Evas_Object *menu_box = elm_box_add(laundry_box);
	evas_object_size_hint_align_set(menu_box, EVAS_HINT_FILL, 0);
	elm_box_horizontal_set(menu_box, EINA_TRUE);
	elm_box_pack_end(laundry_box, menu_box);
	evas_object_show(menu_box);

	Evas_Object *laundry_menu_label = elm_label_add(menu_box);
	elm_object_text_set(laundry_menu_label, "<color=#ABABABFF font_size=50>세탁물 확인</color>");
	elm_box_pack_end(menu_box, laundry_menu_label);
	evas_object_show(laundry_menu_label);

	Evas_Object *laundry_menu_empty = elm_layout_add(menu_box);
	evas_object_size_hint_weight_set(laundry_menu_empty, EVAS_HINT_EXPAND, 0);
	elm_box_pack_end(menu_box, laundry_menu_empty);
	evas_object_show(laundry_menu_empty);

	Evas_Object *expand_btn = elm_button_add(menu_box);
	//elm_object_style_set(expand_btn, "editfield_clear");
	elm_layout_file_set(expand_btn, edj_path, "expand_button");
	Evas_Object *expand_btn_img = elm_image_add(expand_btn);
	elm_image_file_set(expand_btn_img, expand_path, NULL);
//	evas_object_size_hint_min_set(expand_btn_img, 100, 100);
//	evas_object_size_hint_weight_set(expand_btn_img, EVAS_HINT_EXPAND, EVAS_HINT_EXPAND);
//	evas_object_size_hint_align_set(expand_btn_img, EVAS_HINT_FILL, EVAS_HINT_FILL);
	elm_object_part_content_set(expand_btn, "icon", expand_btn_img);
	elm_box_pack_end(menu_box, expand_btn);
	...
	for(int i = 0; i < 2; i++){
		...
		elm_box_padding_set(laundry, 0, 20);
...
//		elm_image_no_scale_set(laundry_img, EINA_TRUE);
//		elm_image_resizable_set(laundry_img, EINA_TRUE, EINA_TRUE);
		elm_image_file_set(laundry_img, laundry_img_path, NULL);
		elm_box_pack_end(laundry, laundry_img);
		evas_object_show(laundry_img);

		Evas_Object *laundry_detail = elm_table_add(laundry);
		evas_object_size_hint_weight_set(laundry_detail, EVAS_HINT_EXPAND, 0);
		evas_object_size_hint_align_set(laundry_detail, 0, 0);
		elm_table_padding_set(laundry_detail, 15, 0);
		elm_box_pack_end(laundry, laundry_detail);
		evas_object_show(laundry_detail);

		for (int row = 0; row < 3; row++) {
			for (int col = 0; col < 2; col++) {
				Evas_Object *laundry_detail_label = elm_label_add(laundry_detail);
				evas_object_size_hint_align_set(laundry_detail_label, 0, 0.5);
				char content[200];
				snprintf(content, 200, detail_css[col], details[i][row][col]);
				elm_object_text_set(laundry_detail_label, content);
				elm_table_pack(laundry_detail, laundry_detail_label, col, row, 1, 1);
